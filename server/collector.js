...
export function normalizeModels(payload) {
 ...
 for(const block of payload.blocks) for(const m of block.models||[]) {
  ...
  unique.set(String(m.id),{sourceId:String(m.id),snapshot_url:rememberSnapshot(m),name:m.username,country:m.country||'未知',viewers:m.viewersCount,online:classifyRoom(m).online,status:classifyRoom(m).status});
 }
  ...
  try {
   ...
   for(const m of models) {
    ...
    const {rows:[saved]}=await db.query(`INSERT INTO models(name,country,language,viewers,growth,online,color,source_id,room_status,last_seen_at)
     VALUES($1,$2,'未知',$3,$4,$5,'#8a6b77',$6,$7,now()) ON CONFLICT(source_id) DO UPDATE SET name=excluded.name,country=excluded.country,viewers=excluded.viewers,growth=excluded.growth,online=excluded.online,room_status=excluded.room_status,room_details=NULL,last_seen_at=now() RETURNING id,monitored`,[m.name,m.country,m.viewers,growth,m.online,m.sourceId,m.status]);
    if(m.online!==null)await db.query('INSERT INTO model_snapshots(model_id,viewers,online) VALUES($1,$2,$3)',[saved.id,m.viewers,m.online]);
    if(old && saved.monitored) for(const rule of rules) {
     const threshold=Number(rule.condition.match(/≥\s*(\d+)/)?.[1]);
     const hit=(rule.condition==='主播上线时' && old.online===false && m.online===true) || (rule.condition.startsWith('观众数 ≥') && threshold>0 && old.viewers<threshold && m.viewers>=threshold);
