...
try{
 ...
 await page.getByRole('button',{name:'触发设置'}).click();await page.getByLabel('人数至少增加').fill('75');await page.getByLabel('目标剩余比例（%）').fill('2');await page.getByRole('button',{name:'保存设置',exact:true}).click();await page.getByRole('button',{name:'触发设置'}).waitFor();assert.equal(config.viewerIncrease,75);assert.equal(config.goalNearPercent,2);
