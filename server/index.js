...
await loadPolling();await startChatMonitor();await startHighlights();await startGoalMonitor();let lastGeneral=0,lastTracked=0,lastAuto=0;const tick=()=>{const now=Date.now(),c=getPolling();if(now-lastGeneral>=c.generalSeconds*1000){lastGeneral=now;void collect().catch(e=>console.error('Collector:',e.message));}if(now-lastTracked>=5000){lastTracked=now;void collectTracked();}if(now-lastAuto>=60000){lastAuto=now;void autoRecord().catch(e=>console.error('Auto recorder:',e.message));}};tick();const timer=setInterval(tick,1000);
let shuttingDown=false;async function shutdown(){if(shuttingDown)return;shuttingDown=true;clearInterval(timer);server.close();await stopGoalMonitor();await stopHighlights();await stopChatMonitor();await browserCapture.disconnect();await shutdownWorker();await pool.end();process.exit();}process.on('SIGINT',shutdown);process.on('SIGTERM',shutdown);
