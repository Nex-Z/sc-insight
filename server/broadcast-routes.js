...
broadcastRoutes.get('/models/:id/broadcasts',async(req,res)=>{
 ...
 pool.query(`SELECT s.*,EXISTS(SELECT 1 FROM chat_coverage cc WHERE cc.model_id=s.model_id AND cc.started_at<=s.last_seen AND coalesce(cc.ended_at,cc.last_seen)>=s.first_seen) AS chat_observed,viewer_sum/nullif(viewer_samples,0) AS average_viewers,
 ...
 sum(i.viewer_average*CASE WHEN i.id IS NOT NULL THEN extract(epoch from least(i.ended_at,(day+interval '1 day') AT TIME ZONE 'Asia/Hong_Kong')-greatest(i.started_at,day AT TIME ZONE 'Asia/Hong_Kong')) END)/nullif(sum(CASE WHEN i.id IS NOT NULL AND i.viewer_average IS NOT NULL THEN extract(epoch from least(i.ended_at,(day+interval '1 day') AT TIME ZONE 'Asia/Hong_Kong')-greatest(i.started_at,day AT TIME ZONE 'Asia/Hong_Kong')) END),0) AS weighted_viewers
