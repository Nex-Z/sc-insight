...
export default function HighlightSettings({data,busy,save,draft,setDraft}){
 const c=data.config;
 const choices=[['viewerRecord','人数变多','观众持续明显增加，保留前 2 分钟到后 3 分钟。'],['tipRecord','打赏变多','短时间打赏激增，或收到大额打赏，保留前 2 分钟到后 3 分钟。'],['goalRecord','目标完成',`目标剩余 ≤ ${c.goalNearPercent}% 或突然完成，录到完成后 15 分钟。`]];
 ...
 return <>
  ...
  <div className="goal-controls"><label className="highlight-toggle"><input type="checkbox" checked={c.goalNotify} disabled={busy} onChange={e=>save(data.enabled,{...c,goalNotify:e.target.checked})}/>目标进度提醒</label><span className="highlight-explainer">快完成或突然完成时提醒，不录制也能开启。</span>{data.goal&&<p className="highlight-explainer">{data.goal.error||Date.now()-new Date(data.goal.updated_at)>30000?'目标数据待更新':data.goal.state?.available?`${data.goal.state.goal?.description||'当前目标'} · ${data.goal.state.completed?'已完成':`剩余 ${Number(data.goal.state.remaining?.toFixed(2))}%`}`:'当前未观察到公开目标'}</p>}</div>
  ...
  {draft&&<form className="highlight-settings" onSubmit={e=>{e.preventDefault();void save(data.enabled,draft);}}>
   <fieldset><legend>人数变多 · 同时满足</legend><p>相对近期平均人数，同时达到以下增幅和增加人数，并持续至少 20 秒。</p><div>{field('viewerRatio','人数增幅（%）',10,1000,0.1,100)}{field('viewerIncrease','人数至少增加',1,1000000)}</div></fieldset>
