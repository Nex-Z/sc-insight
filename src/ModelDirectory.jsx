import useViewportList from './use-viewport-list.js';
...
export default function ModelDirectory({mode,models,Card,cardProps,onResults,initialQuery='',onDiscover}){
 ...
 function reset(){request.current?.abort();setBusy(false);setError('');setQuery('');setOnline('all');setSort('viewers');setRegion('all');setGroup('all');setMin('');setMax('');if(discover)setResult(null);}
 const resetKey=JSON.stringify([mode,discover?result:null,...(!discover?[query,online,sort,region,group,min,max]:[])]);
 const viewport=useViewportList(rows.length,resetKey);
 return <section className="model-directory">
  <form className="panel directory-filters" onSubmit={discover?search:e=>e.preventDefault()}>
   <label>{discover?'官网用户名搜索':'搜索我的关注'}<input aria-label={discover?'官网用户名搜索':'搜索我的关注'} maxLength={80} required={discover} value={query} onChange={e=>{setQuery(e.target.value);}} placeholder="输入主播用户名"/></label>
  ...
  <div className="model-grid" ref={viewport.grid}>{rows.slice(0,viewport.count).map(m=><Card key={m.id} m={m} {...cardProps}/>)}</div>
  ...
  <div ref={viewport.sentinel} className="directory-load-status" role="status">{rows.length>0&&(viewport.count<rows.length?"继续滚动，自动加载更多":"已显示全部 "+rows.length+" 位")}</div>
 </section>;
}
