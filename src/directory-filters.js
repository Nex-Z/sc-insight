export function filterDirectory(models,options={}){
 ...
 return models.filter(m=>(online==='all'||(online==='unknown'?(!m.fresh||m.online==null):m.fresh&&(online==='online'?m.online===true:m.online===false)))&&(region==='all'||m.country===region)&&(group==='all'||(group==='favorite'?m.favorite:m.monitored))&&(min===''||(value(m,'viewers')!==null&&value(m,'viewers')>=Number(min)))&&(max===''||(value(m,'viewers')!==null&&value(m,'viewers')<=Number(max)))).sort((a,b)=>{
