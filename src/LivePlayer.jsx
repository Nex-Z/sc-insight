...
export default function LivePlayer({model,onBack,onDetails,onFollow,onRecord}){
 ...
 useEffect(()=>{
  ...
  (async()=>{try{
   ...
   playback=manageLivePlayback(element,{getHls:()=>hls,onStatus:text=>{if(!disposed)setStatus(text);}});
   if(Hls.isSupported()){hls=new Hls({maxBufferLength:20,backBufferLength:15,liveSyncDurationCount:3,liveMaxLatencyDurationCount:6});hls.loadSource(session.url);hls.attachMedia(element);hls.on(Hls.Events.MANIFEST_PARSED,()=>{if(!disposed)void playback.start();});hls.on(Hls.Events.ERROR,(_,data)=>{if(data.fatal&&!disposed){if(data.type===Hls.ErrorTypes.NETWORK_ERROR)playback.recover();else if(data.type===Hls.ErrorTypes.MEDIA_ERROR)playback.recover(true);else setStatus('直播格式不受支持，请前往官网');}});}
